...
//		Log.i("dawn","访问的url"+url);
    ...
    public <T> void post(final Class<T> clazz, final String url, Header[] header, Map params, RequestDataCallback<T> callback) {
        this.post(clazz, url, header, params, callback, true);
    }
    ...
    public <T> void post(final Class<T> clazz, final String url, Header[] header, final Map params, final RequestDataCallback<T> callback, boolean autoCancel) {
        if (checkAgent()) {
            return;
        }
        add(url, postBuilder(url, header, params, new MyHttpResponseHandler(clazz, url, callback)), autoCancel);
...
//		Log.i("ansen","url:"+url);
    ...
    private Request.Builder getRequestBuild(String url, Map form) {
        if (form == null) {
            form = new HashMap();
        }
